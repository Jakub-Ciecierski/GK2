...
shared_ptr<ID3D11ShaderResourceView> DeviceHelper::CreateShaderResourceView(const wstring& fileName) const
{
	assert(m_deviceObject);
	ID3D11ShaderResourceView* rv;
	auto result = DirectX::CreateWICTextureFromFile(m_deviceObject.get(), fileName.c_str(), nullptr, &rv);
	shared_ptr<ID3D11ShaderResourceView> resourceView(rv, Utils::COMRelease);
	if (FAILED(result))
